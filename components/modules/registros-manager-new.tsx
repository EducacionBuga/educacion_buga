...
export function RegistrosManager({ title, description, areaId }: RegistrosManagerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedFile) {
      toast({
        title: "Error",
        description: "Por favor seleccione una fotografía",
        variant: "destructive",
      })
      return
    }
    ...
    if (!allowedTypes.includes(selectedFile.type)) {
      toast({
        ...
        description: "Solo se permiten imágenes JPG, PNG, GIF y WEBP",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Ocurrió un error al subir el registro fotográfico.",
        variant: "destructive",
      })
    } finally {
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center mt-1 text-xs text-gray-500">
                      ...
                      <span>{format(new Date(registro.date), "dd 'de' MMMM 'de' yyyy", { locale: es })}</span>
                    </div>
                    {registro.location && (
                      <div className="flex items-center mt-1 text-xs text-gray-500">
                    ...
                    {registro.description && (
                      <p className="text-xs text-gray-500 mt-1 line-clamp-2">{registro.description}</p>
                    )}
                  </div>
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                    <Button
                      ...
                      onClick={() => handleView(registro.file_url)}
                      title="Ver imagen"
                    >
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8 rounded-full bg-white/80 shadow-sm"
                      onClick={() => handleDownload(registro.file_url, registro.title)}
